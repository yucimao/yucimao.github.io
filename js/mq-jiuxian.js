document.getElementById("酒仙").innerHTML="<style>#弹窗{position:absolute;left:0;position:fixed;bottom:0;margin:0 0 15px 20px;line-height:18px;padding:7px;width:300px;border-style:solid;border-radius:5px;border:2pxsolid #535353;background-color:#060f27d3;color:#b6b6b6;font-size:12px;z-index:1000;}#弹窗 .标题{color:#e4e4e4;font-weight:bold;font-size:14px;}.提示.灰,#弹窗 .灰{color:#b6b6b6;}#弹窗 .绿{color:#07F602;}#弹窗 .蓝{color:#4646ff}#弹窗 .紫{color:#A827F7}#弹窗 .白{color:#e4e4e4;}#弹窗 .黄,#弹窗 .金{color:#EBBC06;}#弹窗 .红{color:#ff0000;}#弹窗 .酒{color:#23A8F2;}#弹窗 .水{color:#3fc5ff}#弹窗 .使用{color:#CCCCCC;}[onmousemove]{color:#006699;}[onmousemove].x{color:var(--body-color);border-bottom:1.2px solid #08f;}#right_anchor,#right_anchor2{display:none !important}[onmousemove^='进入(']{cursor:default}a[href]{cursor:pointer;}a[href]:after{font-size:0.6em;content:'⤴';color:#bbbbbb;}i{font-size:0.8em;color:#bbbbbb;font-weight:normal;letter-spacing:2px}</style><div id=\"弹窗\" style=\"display:none;\"></div>";
...
let 跳转="";
    ...
    for(let i=0;keys.length>i;i++){
        if(表[keys[i]][0][0]=="酒"){
            ...
            let 临时3=""
            if(表_代码[keys[i]+"_跳转"]){表[keys[i]][表[keys[i]].length]="灰／(点击跳转)";临时3=" href=\"#"+表_代码[keys[i]+"_跳转"]+"\""}
