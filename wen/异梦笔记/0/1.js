...
function 确定() {
    ...
    for (let i=0;sjtk_s.length>i;i++){
        let 过程="0";
        let 关键词=sjtk_s[i].split(",");
        for (let i2=0;输入.length>i2;i2++){
            if (关键词[1].toLowerCase().indexOf(输入[i2]) != -1 && 输入[i2] != "") {}
            else {过程="1";}
        }
        if (过程=="0" && sjtk.indexOf(关键词[0]) != -1){
            结果+="<br><b><a onclick=\"复(制='"+关键词[0]+"')\">"+关键词[0]+"（删除码）</a></b>"+"<iframe src='1/"+关键词[0]+".html' height=100% width=100%></iframe><br>";
        }
    }
